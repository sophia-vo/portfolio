...
(async () => {
  ...
  // Ensure commits array is not empty before creating the scale
  let minDatetime = d3.min(commits, (d) => d.datetime);
  let maxDatetime = d3.max(commits, (d) => d.datetime);

  // Handle case where there might be no commits or only one commit
  if (commits.length === 0) {
    minDatetime = new Date(); // Default to now
    maxDatetime = new Date(); // Default to now
  } else if (commits.length === 1) {
    // If only one commit, set a small range around it for the scale
    minDatetime = new Date(commits[0].datetime.getTime() - 3600 * 1000); // 1 hour before
    maxDatetime = new Date(commits[0].datetime.getTime() + 3600 * 1000); // 1 hour after
  }
  ...
  const timeSliderElement = document.getElementById('commit-progress');
  const commitTimeElement = document.getElementById('commit-time');

  function onTimeSliderChange() {
    if (timeSliderElement && commitTimeElement) {
      commitProgress = +timeSliderElement.value;
      commitMaxTime = timeScale.invert(commitProgress);
      commitTimeElement.textContent = commitMaxTime.toLocaleString(undefined, {
        dateStyle: "long",
        timeStyle: "short"
      });
      // Here you would typically call a function to update your visualization
      // based on the new commitMaxTime.
      // For now, we are just updating the time display.
    }
  }

  if (timeSliderElement && commitTimeElement) {
    timeSliderElement.addEventListener('input', onTimeSliderChange);
    // Initialize the time display on page load
    onTimeSliderChange();
  } else {
    console.error("Slider or time display element not found.");
  }

  renderCommitInfo(data, commits);
  renderScatterPlot(commits); // This function might later use commitMaxTime to filter commits
})();